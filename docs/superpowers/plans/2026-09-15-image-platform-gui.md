...
（24 行 × 24 px，行距 2 px，内边距 4 px），布局落地后内层 grid 的高度
**恒等于该面板的内高**（实测 678 px），不随内容增长 —— 把内容加到 40 行
（按同样参数需要 1046 px）时它仍然是 678 px，超出的部分被直接裁掉，
滚动条够不到。改用绝对定位后，子控件自己的 `Position` 决定内容总高，
面板才能按这个高度滚动。

（以上 678 px 说的是那个实验面板。本平台左栏是另一个面板：默认窗口
1100 × 720 下它的内高是 657 px，而左栏内容总高 595 px（16 个按钮 +
8 个类别标签）—— **放得下、不会触发滚动**。用绝对定位是为了窗口变矮
或以后加算法时能正常滚动，不是现在就需要它。）
